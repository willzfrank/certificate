...
import { MainLayout, Loader, Redirect, AssessmentMobile, WatchCourseMain as WatchCourse } from "app/components";
...
const Index: NextPageWithLayout<{}> = () => {
	...
	const { isLoading, data: courseDetails, isError, error } = useGetSingleCoursePreviewQuery({ courseId }, { skip: !courseId });
	...
	const isCourseInstructor = React.useCallback((): boolean => {
		return courseDetails?.instructors.findIndex((instructor) => instructor.id === userId) !== -1;
	...
	return (
		<WatchCourseContext.Provider value={watchCourseContextInitValues}>
			{page === "assessment" && deviceType !== "desktop" ? <AssessmentMobile moduleId={courseDetails?.modules[activeModuleIndex]?.moduleId as string} /> : <WatchCourse />}
