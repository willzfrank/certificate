...
import { Accordion, AddModuleAccordion, Button, CreateCourseLayout } from "app/components";
...
const CreateCourse_CreateCurriculum: NextPageWithLayout<{}> = () => {
	const { courseInfo, updateCourseInfo, courseModules, updateCourseModules } = React.useContext(CourseCreationContext);
	...
	const [generateModules, { isLoading }] = useCreateNewCourse__GenerateModulesMutation();

	const [mode, courseId] = useWatchSearchParams(["mode", "courseId"]) as string[];
	...
	const [sort, { isLoading: isSortingModule, isError: sortModuleError }] = useSortModuleMutation();
	...
	const onDropCallback = React.useCallback(async () => {
		...
		if (drag.elementId && drop.elementId && drag.position >= 0 && drop.position >= 0 && drag.children && drop.children) {
			try {
	...
	return (
		<React.Fragment>
			...
			<div className="mb-6 md:mb-14 md:w-[65%] w-full">
				<p className="text-2xl md:text-lg font-medium my-1 hidden md:block">Create your currriculum</p>
				<p className="text-muted text-base leading-7 hidden md:block">
					You&apos;ll be able to create, track and manage your courses on this dashboard. You can also track your payments and reviews.
				</p>
			</div>
			...
			<Accordion.Group className="md:my-6 my-10">
				{courseModules.map((module) => (
					<Draggable key={module.moduleId} elementId={module.moduleId} onDropCallback={onDropCallback} position={module.position}>
						<AddModuleAccordion {...module} id={module.moduleId} key={module.moduleId} />
					</Draggable>
				))}
			</Accordion.Group>

			<Link href={mode === "edit" ? `./certificate_and_pricing?mode=edit&courseId=${courseId}` : "./certificate_and_pricing"}>
...
CreateCourse_CreateCurriculum.getLayout = function (page) {
	return <CreateCourseLayout title="Create Curriculum">{page}</CreateCourseLayout>;
};
