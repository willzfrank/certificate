...
import { useCreateNewCourse_CourseInfoMutation, useEditCourseDetailsMutation, useSetCoursePricingMutation } from "app/api/courseCreationApi";
...
const Certificate_and_pricing: NextPageWithLayout<{}> = () => {
	const { courseInfo, updateCourseInfo, updateCertificateRequired, certificateRequired, courseModules, updateCourseModules, pricings, updatePricings } = React.useContext(CourseCreationContext);
	const ddd = React.useContext(CourseCreationContext);
	// console.log(ddd?.pricings);
	const [updateModule, { isLoading: moduleLoading }] = useUpdateModuleMutation();
	...
	const handlePriceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		updatePricings(pricings.map((price: any, index: number) => (index === 0 ? { ...price, price: priceValue } : price)));
	};

	React.useEffect(() => {
		if (courseModules) {
			setCheckboxes(courseModules.map((module) => (module?.paymentRequired ? true : false)));
		}
	}, [courseModules]);
	...
	const toggleCheckbox = async (index: number) => {
		...
		if (res.errors.length === 0) {
			...
			updateCourseModules(courseModules.map((module, i) => (i === index ? { ...module, paymentRequired: !module.paymentRequired } : module)));
		}
	};
	...
	const [setPricing, { error: setPricingError }] = useSetCoursePricingMutation();
	...
	return (
		<div className="lg:py-6 overflow-y-auto overflow-x-hidden no-scrollbar">
			<div>
				<h5 className="text-black text-[15px] font-semibold">Does this course have a Certificate?</h5>
				...
				{/* COURSE PRICING */}
				<div className="mb-5">
					<h5 className="text-black text-[15px] font-semibold mb-1">Course Price</h5>
					<div>
						<input
				...
				<div className="mb-6 lg:w-full h-[189.08px] lg:h-full   md:w-[65%] overflow-x-auto no-scrollbar">
					<h5 className="text-black text-[15px] font-semibold mb-1 h-[50px]">Course Content</h5>
										...
										<div className="px-4 py-3 h-[90px] flex justify-center flex-col items-center">
											<div className="flex gap-2">
												...
												<label className="relative inline-flex items-center mb-5 cursor-pointer">
													<input type="checkbox" value={""} checked={checkboxes[index]} onChange={() => toggleCheckbox(index)} className="sr-only peer" />
...
"
			...
			<div>
				<button className="text-app-pink border-2 border-app-pink px-8 py-2 rounded" onClick={onSubmit}>
					Next
				</button>
...
Certificate_and_pricing.getLayout = function (page) {
	return <CreateCourseLayout title="Certificate and Pricing">{page}</CreateCourseLayout>;
};
