...
const CreateCourse_CourseInfo: NextPageWithLayout<{}> = () => {
  ...
  const { updateCourseInfo, courseInfo } = React.useContext(
  ...
  const [mode, editedCourseId] = useWatchSearchParams(['mode', 'courseId']) as [
    string,
    string
  ];
    ...
    if (mode === 'edit') {
      ...
      if (data) {
        updateCourseInfo({
          ...
          targetAudience: data.data.intendedUsers,
          categories: data.data.categories,
          subtitle: String(data.data.subTitle),
        ...
        if (courseInfo.isExternal) {
          ...
          router.push(
            `./createcurriculum?mode=edit&courseId=${editedCourseId}`
          );
        }
      } else {
        // handle error
...
const CreateCourseFormField = ({
  ...
  return (
    <div className="mb-4">
      ...
      <p
        id={`descriptionFor${inputElementId}`}
        className="text-muted text-[15px] lg:text-base leading-6 lg:leading-7 w-[95%] lg:w-full"
      >
        {desc}
      </p>
      {inputElement}
