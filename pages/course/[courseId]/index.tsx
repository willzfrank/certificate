...
import { AssessmentMobile, Loader, MainLayout, Redirect, WatchCourseMain } from "app/components";
...
const Index: NextPageWithLayout<{}> = () => {
	...
	const [deviceType, setDeviceType] = React.useState<ReturnType<typeof getDeviceType>>();

	const [loading, setLoading] = React.useState(true);

	const courseId = useWatchSearchParams("courseId") as string;

	const [getCourseDetails, { isLoading, data: courseDetails, isError, error }] = useLazyGetSingleCoursePreviewQuery();
	...
	React.useEffect(() => {
		(async function () {
			if (courseId) {
				...
				if (res.isExternal && confirm("Are you sure you want to redirect to " + res.redirectUrl)) {
	...
	const [activeResourceType, setActiveResourceType] = React.useState<ModuleContentTypes>(ModuleContentTypes.video);
	const [activeResourceIndex, setActiveResourceIndex] = React.useState<number>(0);
	...
	const watchCourseContextInitValues: React.ContextType<typeof WatchCourseContext> = {
	...
	if (courseDetails && !courseDetails.isSubscribed && !freeModules) {
		return (
			<div className="h-[85vh] w-[90vw] mx-auto flex items-center justify-center">
				<Loader mainColor="red" className="w-24 h-24" />
	...
	return (
		<WatchCourseContext.Provider value={watchCourseContextInitValues}>
			{page === "assessment" && deviceType !== "desktop" ? <AssessmentMobile moduleId={courseDetails?.modules[activeModuleIndex]?.moduleId as string} /> : <WatchCourseMain />}
