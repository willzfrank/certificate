...
pdfjs.GlobalWorkerOptions.workerSrc = "https://firebasestorage.googleapis.com/v0/b/certs-by-unify.appspot.com/o/pdfworkerbuild.js?alt=media&token=03171bab-2901-4c5d-ab07-7954ddcf9257";
...
const PDFError = (props: { errorType: "load" | "source" }) => {
	return <p className="text-white">AN ERROR OCCURRED WHILE LOADING PDF {props.errorType}</p>;
};

const PDFViewerModal = (props: PDFViewerModalProps) => {
	const [errorType, setErrorType] = useState<React.ComponentProps<typeof PDFError>["errorType"]>("load");
	...
	useEffect(() => {
		const handleResize = () => {
			...
			const isMediumMobile = window.innerWidth >= 380 && window.innerWidth < 427;
	...
	return (
		<div className="mb-10 md:w-full  min-h-max">
			<div className="p-6 bg-[#1c1b1b] w-full  text-white " style={{ boxShadow: "0px 5px 10px #2b2a2a" }}>
			...
			<div className={`h-[93vh] md:h-[90vh] overflow-y-scroll ${props.className}  mb-8 relative w-full pdf-viewer`}>
				<Document
					...
					{Array.from({ length: numberofPages }, (_, index) => (
						<Page className="mx-auto w-fit my-4 overflow-scroll" key={`page-${index + 1}`} pageNumber={index + 1} scale={scale} renderAnnotationLayer={true} renderTextLayer={true} />
					))}
				</Document>
