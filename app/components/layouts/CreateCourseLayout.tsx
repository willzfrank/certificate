  ...
  return (
    <div className="flex px-4 md:px-14 items-center justify-between py-3 md:py-4 bg-app-stroke sticky top-0 z-10">
      ...
      <p className="font-medium">
        Create new {isExternal ? 'external' : ''} course
      </p>
      <Link href="/instructors/overview">
        <button className="border-2 border-app-pink px-6 py-2 rounded text-app-pink">
          {btnText}
        </button>
      </Link>
    </div>
...
const CreateCourseLayout = ({
  ...
  let createExternalCourse = useWatchSearchParams(
    'isExternal'
  ...
  const [courseInfo, setCourseInfo] = React.useState<
    CourseCreationContextFields['courseInfo']
  >({
    id: '',
    courseName: '',
    subtitle: '',
  ...
  const [isCertificateRequired, setIsCertificateRequired] = React.useState<
    CourseCreationContextFields['isCertificateRequired']
  >({
    price: 0,
    isCertified: true,
  });

  const [courseMedia, setCourseMedia] = React.useState<
    CourseCreationContextFields['courseMedia']
  >({
    videoPreview: '',
    imagePreview: '',
  });
  ...
  const [courseModules, setCourseModules] = React.useState<
    CourseCreationContextFields['courseModules']
  >([]);

  const [getCourseDetails, { isFetching, data }] =
    useLazyGetCoursePreviewWhileCreatingQuery();
  ...
  const fetchCourseDetailsAndUpdateContext = async (id: string) => {
    ...
    setCourseInfo({
      ...
      targetAudience: course.intendedUsers,
      lastSavePoint: course.lastSavePoint,
      redirectUrl: course.redirectUrl,
      isExternal: course.isExternal,
    });

    setCourseMedia({
      videoPreview: course.previewVideoUrl,
  ...
  const [courseId, mode] = useWatchSearchParams([
    'courseId',
    'mode',
  ]) as string[];
    ...
    >
      <CourseCreationContext.Provider
        value={{
          ...
          updateIsCertificateRequired: (newValues) =>
            setIsCertificateRequired(newValues),
          updateCourseModules: (updatedCourseModules) =>
      ...
      >
        ...
        <div className="grid md:grid-cols-[300px,_1fr]">
          <div className="h-full pl-14 py-8 pr-2 space-y-6 border-r hidden md:block">
            {links.map((link, index) => (
              <div
                key={index}
                className={`${
                  pathname.split('/')[3] === link.url
                    ? 'text-app-pink font-semibold'
                    : ''
                }`}
              >
                {index <= courseInfo.lastSavePoint ? (
                  <Link
                    href={`./${link.url}${
                      mode === 'edit' ? '?mode=edit&courseId=' + courseId : ''
                    }`}
                  >
                    <a>{link.title}</a>
                  </Link>
          ...
          <div className={noPadding ? '' : 'px-6 md:px-14 py-6'}>
            {title && (
              <p
                className={`text-2xl md:hidden font-medium ${
                  noPadding ? 'my-6 px-6' : 'mb-6'
                }`}
              >
                {title}
              </p>
