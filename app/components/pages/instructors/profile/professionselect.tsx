...
const ProfessionSelectField = ({
  professionList,
  label,
  index,
  updateProfessions,
  allUserProfessions,
}: ProfessionSelectType) => {
  const [showDropdown, setShowDropdown] = useState(false);
  const professions = useMemo(() => {
    if (allUserProfessions?.length === 0) return;

    const newProfessionValue = allUserProfessions[index]?.name;
    if (newProfessionValue === '' || !newProfessionValue)
      return [...professionList];
    return professionList.filter((pro: any) =>
      pro?.name?.toLowerCase().startsWith(newProfessionValue?.toLowerCase())
    );
    ...
    <div className="mb-5 w-full form-inputs mt-4 relative md:mt-0">
      ...
      >
        <input
          ...
          value={
            allUserProfessions[index] ? allUserProfessions[index]?.name : ''
          }
      ...
      {showDropdown && professions?.length > 0 && (
        <ul className="w-full h-auto absolute bg-white top-[100%] z-[3] overflow-hidden list-none mt-[2px] rounded border-black border-[1px]">
          {professions?.map((profession: any) => (
            <li
              className="capitalize px-2 py-[4px] hover:text-white hover:bg-blue-700"
              key={profession.id}
              value={profession.id}
