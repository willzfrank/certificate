...
import ProfessionSelectField from "./professionselect";

export interface UserForm extends Omit<USERMODEL, "profilePictureUrl"> {
...
const validateImage = (imageFile?: File) => {
	if (imageFile && imageFile.name.match(/.(jpg|jpeg|png|gif|webp|svg)$/i) && imageFile.size / 1000 < 100) {
		return true;
	}
	return false;
};
...
const getProfessiontToAddOrRemove = (old: string[], current: string[]) => {
	...
	let profs: Record<string, boolean> = old.reduce((list: Record<string, boolean>, prof: string) => {
		list[prof] = false;
		return list;
	}, {});
...
const BasicInfoTab = () => {
	...
	const { data: professionList, isLoading: professionListLoading, isError: professionListErr } = useGetProfessionsQuery();
	const [updateInstructorProfile, { isLoading, error, isError, isSuccess }] = useUpdateInstructorProfileMutation();

	const [updateInstructorPicture, { isLoading: pictureLoading, error: pictureError, isError: pictureisError, isSuccess: pictureSuccess }] = useUpdateInstructorPictureMutation();
	...
	function updateProfessions(index, newProfessionValue) {
		if (!profile) return;
		let newProfessions = profile.professions ?? [];
		// Check to see if the new value is part of our professions or not
		let changedProfessionId = professionList?.find((profession: any) => newProfessionValue.toLowerCase() === profession?.name?.toLowerCase());

		if (newProfessions[index]) {
			newProfessions = newProfessions.map((id, i) => {
				if (index === i) {
					return { name: newProfessionValue, id: changedProfessionId?.id || "" };
				}
				return id;
			});
		} else {
			newProfessions = [...newProfessions, { name: newProfessionValue, id: changedProfessionId?.id || "" }];
		}

		setProfile((prev) => ({ ...prev, professions: newProfessions }));
	}

	const onChange = (field: keyof InstructorModel) => (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
		if (profile) {
	...
	const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			if (profile?.profileImageFile) {
				...
				formData.append("imageFile", profile.profileImageFile, profile.profileImageFile.name);
				...
				dispatch(userSlice.actions.updateUserProfilePicture(res.profilePictureUrl));
			}

			// Find professions that are not stored in database
			const uniqueProfessions = profile?.professions.filter((profession) => {
				let found = professionList.find((item) => item?.name?.toLowerCase() == profession.name.toLowerCase());
				return !found;
			});
			console.log("Professions that are not stored in the DB", uniqueProfessions);

			// const { toAdd, toRemove } = getProfessiontToAddOrRemove([...new Set(profile?.oldProfessions)], [...new Set(profile?.professionIds)]);
	...
	const userProfessions = useMemo(() => {
		if (profile?.professions?.length >= 3) return profile?.professions;
		let professions = profile?.professions;
		for (const i = profile?.professions?.length; i < 3; i++) {
	...
	return (
		<form onSubmit={onSubmit} className="pb-24">
			...
			<div className="md:flex w-full md:space-x-32 mt-6 mb-14">
				<div className="font-medium text-lg mt-7 md:mt-0 md:min-w-[153px]">Instructor image</div>
				<div className="space-y-3 mt-4 md:mt-0">
					<div className="relative w-36 aspect-square">
						{profile?.profilePictureUrl || user?.profilePictureUrl ? (
							<Image
								src={getUrl(profile?.profilePictureUrl || user?.profilePictureUrl, profile?.profileImageFile)}
							...
							<svg xmlns="http://www.w3.org/2000/svg" width="142" height="103" fill="none" viewBox="0 0 142 103">
								<rect width="140.096" height="102" x="0.5" y="0.5" fill="#F7EFF4" rx="5.14"></rect>
								<path
									fill="#C5B8C9"
									fillRule="evenodd"
								...
								<rect width="140.096" height="102" x="0.5" y="0.5" stroke="#ADADAD" rx="5.14"></rect>
							</svg>
						)}
					</div>
					<div>
						<div className="border rounded-[4px] overflow-hidden flex items-stretch justify-between md:min-w-[400px] pl-4">
							<input type="file" placeholder="No image uploaded" className="opacity-0" id="file" accept="image/*" onChange={onChange("profilePictureUrl")} />

							<label htmlFor="file" className="border-2 border-app-gray-400 px-4 rounded-[4px] py-[6px] font-normal text-base">
								Select file
							</label>
						</div>
						{fileError ? (
							<p className="text-app-pink text-sm absolute italic py-1">
								<strong>Invalid File!</strong> File must be less than 100kb and must end in either .jpeg, .jpg, .svg, .png, .webp or .gif
							</p>
						) : null}
					</div>
				</div>
			...
			<div className="mt-10 w-full md:space-x-32 md:flex">
				<div className="text-lg font-medium md:min-w-[153px]">Personal data</div>
				<div>
					{" "}
					<form className="bg-white rounded w-full">
						<div className="mb-5 form-inputs mt-4 md:mt-0">
							<label className="block mb-2 font-normal text-base">First name</label>
							<input
								id="text"
								type="text"
								name="text"
						...
						<div className="mb-5 form-inputs">
							<label className="block mb-2 font-normal text-base">Last name</label>
							<input
								id="text"
								type="text"
								name="text"
			...
			<div className="mt-10 w-full md:space-x-32 md:flex">
				...
				<div>
					...
					{professionList?.length && userProfessions?.length ? (
						<div className="bg-white rounded w-full">
							{userProfessions?.map((item, index) => (
								<ProfessionSelectField
									key={index}
									label={`Profession ${index + 1}`}
									professionList={professionList}
									updateProfessions={updateProfessions}
									defaultValue={item?.name ? item?.name.toLowerCase() : ""}
									allUserProfessions={userProfessions}
									index={index}
								/>
							))}
						</div>
