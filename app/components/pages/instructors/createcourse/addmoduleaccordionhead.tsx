...
const AddModuleTypeAccordion__Head = ({
  ...
  const { courseInfo, updateCourseModules } = React.useContext(
    CourseCreationContext
  )

  const [getCourseDetails, { isFetching: isFetchingNewCourseDetails }] =
    useLazyGetCoursePreviewWhileCreatingQuery()
  ...
  const handleEditModuleName = React.useCallback(async () => {
    if (editingModuleName && editedModuleName) {
      //   setEditedModuleName(editedModuleName)
      const res = await updateModule({
        ...
        moduleDetails: {
          ...
          paymentRequired: moduleDetails?.paymentRequired
            ? moduleDetails.paymentRequired
            : true,
        },
      }).unwrap()
  ...
  return (
    <div className="py-2 flex items-center justify-between">
      <div
        className="flex md:gap-8 gap-4 items-center"
      ...
      >
        {editingModuleName ? (
          <input
            ...
            onChange={(e) => setEditedModuleName(e.currentTarget.value)}
            className="w-[200px] text-sm p-2 rounded"
          />
        ) : (
          <p>{editedModuleName}</p>
        )}
        <div className="flex items-center gap-3">
          <button
            className={`w-8 h-8 rounded-full flex items-center justify-center ${
              editingModuleName ? 'bg-green-400 bg-opacity-25' : 'bg-app-stroke'
            }`}
            onClick={(e) => {
              e.stopPropagation()
          ...
          <Button
            ...
            className="w-8 !h-8 rounded-full bg-[#F6E9ED] flex items-center justify-center"
      ...
      <div className="flex items-center gap-8">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            ...
            >
              <path
                d="M12.6654 6.16699L7.9987 10.8337L3.33203 6.16699"
                stroke="#B61046"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
          ...
          <AnimatePresence>
            {showResourceTypeDropdown && (
              ...
              >
                ...
                >
                  <button ref={buttonRef0} className="px-4 pl-6 py-3 text-left">
                    Upload Video
                  </button>
                </StopPropagation>
                <StopPropagation<HTMLButtonElement>
                  ...
                  callback={() => {
                    setResourceType(ModuleContentTypes.youtube)
                    openAccordion()
                  }}
                >
