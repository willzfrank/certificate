...
const AddModuleAccordion__Body = ({
  ...
  const [deleteAssessment, { isLoading: isDeletingAssessment }] =
    useDeleteAssessmentMutation();
  ...
  const [removeVideo, { isLoading: isRemovingVideo }] =
    useCreateNewCourse__RemoveVideoFromModuleMutation();

  const [removeDocument, { isLoading: isRemovingDocument }] =
    useCreateNewCourse__RemoveDocumentFromModuleMutation();
  ...
  const [removingDocumentId, setRemovingDocumentId] =
    React.useState<string>('');
  const [removingInteractiveId, setRemovingInteractiveId] =
    React.useState<string>('');

  const toggleVideoEdit = (video: VideoResourceType) => {
    const editedResources = allResources.map((_resource) => {
      if (
        resourceguards.isVideoResource(_resource) &&
        _resource.value.id === video.value.id
      ) {
        return {
  ...
  const toggleDocumentEdit = (document: DocumentResourceType) => {
    const editedResources = allResources.map((_resource) => {
      if (
        resourceguards.isDocumentResource(_resource) &&
        _resource.value.id === document.value.id
      ) {
        return {
  ...
  const toggleInteractiveEdit = (interactive: {
    isInputing: boolean;
    value: AllModuleResources[number];
  }) => {
    const editedResources = allResources.map((_resource) => {
      if (
        (isThisOrThatResource(_resource) &&
          _resource.value.position === interactive.value.position) ||
        interactive.value.position === _resource.value.position
      ) {
        return {
  ...
  const [
    ...
    { isLoading: isDeletingInteractiveContent },
  ] = useDeleteInteractiveTypeMutation();
      ...
      {allResources.map((resource) => {
        if (resourceguards.isVideoResource(resource)) {
              ...
              >
                <div className="px-4 md:px-6 py-3 flex justify-between items-center text-sm text-gray-600 bg-inherit">
                  <div className="flex gap-8">
                    ...
                    <p>
                      {resource.value.totalSeconds} seconds of video content
                      uploaded
                    </p>
                  </div>
                  <div className="flex items-center gap-3 justify-end">
                    <Button
                      ...
                      onClick={() => toggleVideoEdit(resource)}
                    >
                      <svg
                        width="32"
                        height="32"
                    ...
                    <Button
                      loading={
                        isRemovingVideo && removingVideoId === resource.value.id
                      }
                      onClick={() => {
            ...
            return (
              ...
              >
                <div className="px-4 md:px-6 py-3 flex justify-between items-center text-sm text-gray-600">
                  <div className="flex gap-8">
                    <p className="w-[250px] truncate">
                      Document: {resource.value.displayName}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-3 justify-end">
                    <Button
                      className="w-10 !h-10 rounded-full flex items-center justify-center bg-app-stroke"
                      onClick={() => toggleDocumentEdit(resource)}
                    >
                      <svg
                        width="32"
                        height="32"
                    ...
                    <Button
                      className="w-10 !h-10 rounded-full bg-[#F6E9ED] flex items-center justify-center "
                      loading={
                        removingDocumentId === resource.value.id &&
                        isRemovingDocument
                      }
                      onClick={() => {
          ...
          return (
            ...
            >
              <div className="px-4 md:px-6 py-3 flex justify-between items-center text-sm text-gray-600">
                Click and Match Interactive Type with{' '}
                {resource.value.statements.length} statements added
                <div className="flex items-center gap-3 justify-end">
                  <Button
                    className="w-10 !h-10 rounded-full flex items-center justify-center bg-app-stroke"
                    onClick={() => toggleInteractiveEdit(resource)}
                  >
                    <svg
                      width="32"
                      height="32"
                  ...
                  <Button
                    className="w-10 !h-10 rounded-full bg-[#F6E9ED] flex items-center justify-center"
                    loading={
                      removingInteractiveId === resource.value.id &&
                      isDeletingInteractiveContent
                    }
                    onClick={() =>
          ...
          return (
            ...
            >
              <div className="px-4 md:px-6 py-3 flex justify-between items-center text-sm text-gray-600">
                This or That Interactive Resource with{' '}
                {resource.value.questions.length} questions
                <div className="flex items-center gap-3 justify-end">
                  <Button
                    className="w-10 !h-10 rounded-full flex items-center justify-center bg-app-stroke"
                    onClick={() => toggleInteractiveEdit(resource)}
      ...
      {isEmpty && currentResourceType === ModuleContentTypes.NULL && (
        <div className="px-4 min-h-[200px] flex items-center justify-center text-center flex-col">
          ...
          <p className="text-base w-1/3">
            You can add videos, assessments, documents or interactive modules.
          </p>
        </div>
      )}
    </div>
...
const WithDND = (
  props: React.ComponentProps<typeof AddModuleAccordion__Body>
) => {
  return (
