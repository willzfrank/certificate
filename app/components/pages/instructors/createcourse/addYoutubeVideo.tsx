...
const AddYoutubeVideo = ({
  ...
  const [addVideo, { isLoading: isVideoLoading }] =
    useCreateNewCourse__AddVideoToModuleMutation()

  const [editVideo, { isLoading: isEditVideoLoading }] =
    useEditModuleVideoMutation()
  ...
  const videoSubmit: SubmitHandler<any> = async (data: {
    ...
    if (startFrom && +startFrom >= +endAt) {
      // Display an error alert or handle the error appropriately
      notify({
        title: 'Error',
        description: 'Start time must be less than stop time',
        type: 'error',
      })
      return
    }
        ...
        <div className="w-1/2 mt-3 h-auto gap-5 flex justify-start items-center">
          <div className="w-1/2 h-full">
            ...
            <input
              ...
              {...formControl.register('startFrom')}
            />
          </div>
          <div className="w-1/2 h-full">
