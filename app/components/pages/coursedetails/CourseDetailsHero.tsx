...
export const calculateDiscountedPrice = (price: number, discountDetails: DiscountDetailsType) => {
	let updatedPrice = price;

	if (discountDetails.hasAppliedDiscount) {
		updatedPrice = discountDetails.type === "DiscountByAbsoluteValue" ? discountDetails.value : price - Math.floor(price * (discountDetails.value / 100));
	}
				...
				<div className="flex flex-col lg:flex-row lg:divide-x-2 lg:divide-[#2E284A] h-full items-stretch flex-1">
										...
										<p className="font-semibold">
											{discountDetails.hasAppliedDiscount ? (
												<span className="inline-flex gap-4 ml-2">
													<span className="text-white line-through">₦ {props.pricings[0].price}</span>
													<span className="text-white">
														₦
														{discountDetails.type === "DiscountByAbsoluteValue"
															? discountDetails.value
															: props.pricings[0].price - Math.floor(props.pricings[0].price * (discountDetails.value / 100))}
													</span>
												</span>
											) : (
												<span className="text-white">₦ {props.pricings[0].price.toLocaleString()}</span>
											)}
										</p>
										<div className="ml-0">
											{props.pricings[0].price === 0 ? (
												...
												<p className=" h-3 cursor-pointer text-orange-500 text-[10px] md:text-[13px] font-medium underline" onClick={openDiscountModal}>
													Use Discount Code
												</p>
											)}
											{(!isSubscribed || !props.isSubscribed) && (
												<div className={`${isSubscribed || props.isSubscribed ? "cursor-not-allowed" : "cursor-pointer"}`}>
													...
													<Modal isOpen={discountModal} closeModal={closeDiscountModal}>
														<DiscountModal
                 ...
                 text-[#06C281] border-2 inline-flex items-center justify-center space-x-4
                  border-[#06C281] font-bold">
								...
								<>
									{showAccessButton && freeModulesAvailable && (
										<button
											...
											{pricingPlan.price === 0 ? (
												<div onClick={handlePay(pricingPlan?.price, pricingPlan?.subscriptionType, pricingPlan?.name, pricingPlan?.id)}>
													<div>
														<p>Enroll For Free</p>
												...
												<p className="text-center text-blue-500 md:text-lg text-base font-bold">Start Course</p>
											)}
										</button>
									)}
									{showAccessButton && freeModulesAvailable && (
							...
							{/* BUY NOW BUTTON */}
							{(accessCourse || !freeModulesAvailable) && pricingPlan?.price > 0 && (
									...
									{!isSubscribed && !props.isSubscribed && (
										<div className="w-full h-[56px] flex items-center justify-center bg-red-600 cursor-pointer rounded-[35.06px] transition-transform md:mt-0 mt-5 hover:scale-105">
											<Button
												...
												Buy Now
												<small className="text-white text-[10px] font-semibold">Access all the contents of this course</small>
											</Button>
										</div>
									)}
									...
									{freeModulesAvailable && (
										<Link href={`/course/${props.id}?slugName=${props.slugName}`}>
											<div className="w-full h-[56px] flex items-center flex-col  justify-center gap-1 rounded-[35.06px] border border-red-600 cursor-pointer transition-transform hover:scale-105">
												<p className="text-white  text-[12.88px] md:text-[15.88px] font-semibold">Learn For Free</p>
												<small className="text-white text-[10px] font-semibold">Access the free contents of this course</small>
											</div>
										</Link>
									)}
								</div>
