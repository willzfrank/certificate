...
const CourseContent = ({
  ...
  return (
    ...
    >
      {!hideTitle ? (
        <p className="font-medium text-xl mb-4">Course Content</p>
      ) : null}
      <Accordion.Group>
          ...
          return (
            <ModuleDetailsAccordion
              {...{
                ...
                courseId,
                moduleName: `Module ${index + 1}: ${module.name}`,
                paymentRequired:
                  module.paymentRequired == undefined
                    ? true
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-muted text-[15px]">
                {getTime(
                  (moduleContent?.data.totalSeconds || 0) * 1000,
                  'MM mins',
                  'absolute'
                )}
              </p>
          ...
          <div>
            {mergedList.map((resource, index) => {
              if (resourceguards.isVideo(resource)) {
                return (
                  <div
                    className="grid items-center gap-2 grid-cols-[minmax(20px,auto)_minmax(0,1fr)_minmax(80px,auto)] text-muted my-4 w-full"
                    key={'videoe-' + index}
                  >
                    <div
                      className={`w-5 h-5 items-center justify-center flex rounded-full border border-app-dark-500`}
                    >
                      <svg
                ...
                return (
                  <div
                    className="grid items-center gap-2 grid-cols-[minmax(20px,auto)_minmax(0,1fr)] text-muted my-4 w-full"
                    key={'document' + index}
                  >
                    <div
                      className={
                        'w-5 h-5 items-center justify-center flex rounded-full border'
                      }
                    >
                      <svg
                        className="scale-150"
