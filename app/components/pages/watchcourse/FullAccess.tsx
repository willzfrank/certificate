...
interface Props {
	...
	calculateDiscountedPrice: (price: number, discountDetails: DiscountDetailsType, setPricingPlan: any) => number;
...
const FullAccess = (props: Props) => {
	...
	const [addSubscription, { isLoading: isCheckingSubscription, error, isError, isSuccess }] = useAddSubscriptionMutation();
	...
	const applyDiscountCode = React.useCallback(async () => {
		try {
			const res = await checkCodeValidity(props.discountDetails.discountCode).unwrap();
			if (res.data.isValid) {
		...
		if (price === 0) {
			try {
				const {
					...
					channel: "Card",
					source: "Web",
					discountCode: props.discountDetails.hasAppliedDiscount ? props.discountDetails.discountCode : undefined,
		...
		if (status === "ready" && window) {
			...
			try {
				...
				if (error?.status === 400 || error?.status === 401) {
					...
					notify({
						title: "Couldn't enroll for this course.",
						description: "Please create an account to enroll for this course.",
						type: "error",
					});
				} else {
					notify({
	...
	return (
		<div className="flex items-center flex-col relative  bg-zinc-300  rounded p-10 md:px-20 md:py-12">
			<div>
				<div className="text-black md:text-[25px] font-bold text-center font-['Inter']">Get Full Access!</div>
				<svg
				...
				<div className=" text-black sm:text-xs md:text-lg font-medium text-center font-['Inter']">Subscribe for this course to get full access</div>
				<div className="flex mt-10 items-center gap-2">
					{/* DISCOUNT HERE */}
					<div className="space-y-4 w-full">
						{!isApplyingDiscountCode && user.id && (
							<button className=" md:w-[138px] h-11 bg-white rounded-[10px] border  border-red-600" onClick={() => HideSubscriptionButtonFunction()}>
								<span className="text-red-600 text-[13px] md:px-0 px-2 font-medium font-['Inter']">Use Discount Code</span>{" "}
							</button>
						)}
						{isApplyingDiscountCode && !props.discountDetails.hasAppliedDiscount && (
							<div className="overflow-clip ">
								<motion.div className=" space-x-1 rounded-lg p-1 text-muted h-14 flex" animate={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 60 }}>
									<input
									...
									<Button
										...
										<span className="md:w-[60px] text-white text-lg font-semibold font-['Inter']">APPLY</span>{" "}
									</Button>
								</motion.div>
							</div>
						)}
					...
					{hideSubscribeButton && (
						<div className=" flex items-center justify-center  ">
							<Button
								...
								onClick={handlePay(props.pricingPlan?.price, props.pricingPlan?.subscriptionType, props.pricingPlan?.name, props.pricingPlan?.id)}
								loading={isCheckingSubscription}>
								<div>
									<div className=" text-white md:text-lg text-[18px] font-semibold rounded font-['Inter']">Subscribe</div>
								</div>
								<div className="  text-white text-center md:text-xs text-[12px] font-medium font-['Inter']">
									{props.pricingPlan.price > 0 ? <span>₦{props.pricingPlan.price.toLocaleString()}</span> : <span>₦0</span>}
								</div>
							</Button>
						</div>
					)}
						...
						<div className=" flex items-center justify-center w-full">
							<Button
								...
								<div className="flex items-center">
									<span className=" text-white text-lg font-semibold font-['Inter']">Pay</span>
												...
												<div className="flex-1">
													<div className="inline-flex gap-4 ml-2">
														<span className="text-white">
															₦
															{props.discountDetails.type === "DiscountByAbsoluteValue"
																? props.discountDetails.value.toLocaleString()
																: (props.pricings[0].price - Math.floor(props.pricings[0].price * (props.discountDetails.value / 100))).toLocaleString()}
														</span>
													</div>
												</div>
											</div>
