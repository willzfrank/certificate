...
const Certificate = ({ courseName, courseId, close, onClick }: CertProps) => {

  React.useEffect(() => {
    const localCompletedCourse = JSON.parse(
    ...
    if (!localCompletedCourse.includes(courseId)) {
      ...
      localStorage.setItem(
        LOCAL_COMPLETED_COURSES,
        JSON.stringify(localCompletedCourse),
      )
    }
      ...
      <div className="border p-5">
        <div className="flex">
          ...
          <div className="ml-6 space-y-1">
            <p className="font-medium text-lg text-app-dark-400">
              You’ve successully completed {courseName}
            </p>
            <p className="text-[15px] text-muted">
              You can find a link to your certificate in your email
            </p>
          </div>
        </div>
      </div>
      <div className="border p-5 ">
        <div className="flex">
          ...
          <div className="ml-6 space-y-3">
            <p className="font-medium text-lg text-app-dark-400">
              Let us know what you think about this course
            </p>

            <button
              onClick={onClick as MouseEventHandler}
              type="button"
              className="px-7 py-2 border border-[#55565C] rounded-md bg-white"
            >
              Rate course
            </button>
          </div>
        ...
        <div className="flex">
          ...
          <div className="ml-6 space-y-8 mb-4">
            <div className="space-y-2">
              <p className="font-medium text-lg text-app-dark-400">
                {' '}
                Want to advance your skill?
              </p>
              <p className="md:w-4/5 text-app-dark-400">
                Congratulations on completing {courseName}, here are a few more
                courses to keep you on the path of success. More courses in
                “Course category goes here”:
              </p>
            </div>
            <div>
