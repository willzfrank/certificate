...
import { PricingPlan, RootState } from "app/types";
...
import { useRegisterDocumentAsReadMutation, useRegisterVideoAsWatchedMutation } from "app/api/subscriptionApi";
import * as resourceGuards from "app/types/guards";
import { QuestionOptionType, ModuleContentTypes, USERTYPES } from "app/types";
import { useWatchSearchParams, useAppSelector, useCourseCompletion, useNotify } from "app/hooks";
...
import PDFViewerHeader from "app/components/elements/PDFViewerHeader";
import WatchCourseMainAccess from "./WatchCourseMainAccess";
...
const WatchCourseMain = () => {
	...
	const [registerVideo, { isLoading: isRegisteringVideo, isError: isRegisteringError, isSuccess: isRegisteringSuccess }] = useRegisterVideoAsWatchedMutation();
	...
	const [pricingPlan, setPricingPlan] = React.useState<PricingPlan>(courseDetails?.pricings[0] ?? freePlan);

	const calculateDiscountedPrice = (price: number, discountDetails: DiscountDetailsType) => {
		let updatedPrice = courseDetails?.pricings[0].price;

		if (discountDetails?.hasAppliedDiscount) {
			const originalPrice = courseDetails?.pricings[0]?.price ?? 0; // Use a default value if courseDetails is undefined

			updatedPrice = discountDetails.type === "DiscountByAbsoluteValue" ? discountDetails.value : originalPrice - Math.floor(originalPrice * (discountDetails.value / 100) ?? 0); // Use a default value if discountDetails or discountDetails.value is undefined
		}
	...
	const moveToNextResource = React.useCallback(() => {
		...
		if (activeResourceIndex === allResourses.length - 1) {
			...
			if (resourceGuards.isAssessment(newActiveResource)) {
				router.push(`${roleName?.toLowerCase() === USERTYPES.INSTRUCTOR ? "/instructors" : ""}/course/${courseId}/?page=assessment&assessmentId=${newActiveResource.id}`);
			} else if (
				...
				router.push(`${roleName?.toLowerCase() === USERTYPES.INSTRUCTOR ? "/instructors" : ""}/course/${courseId}/?page=interactivepreview`);
	...
	}, [activeResource, roleName, setActiveResourceIndex, activeResourceIndex, studentId, courseDetails?.modules, activeModuleIndex]);

	let [isOpen, setIsOpen] = React.useState(false);
	const [shouldShowCertificate, setShouldShowCertificate] = React.useState(false);

	const courseCompletionData = useCourseCompletion(courseId, courseDetails?.modules.map((module) => module.moduleId) || [""], shouldShowCertificate);

	React.useEffect(() => {
		setShouldShowCertificate(courseCompletionData?.isCompleted);
	}, [courseCompletionData?.isCompleted]);
	...
	const [registerDocumentAsRead, { isLoading: isRegisteringDocumentAsRead, error: registerDocumentAsReadError }] = useRegisterDocumentAsReadMutation();

	const [rateCourse, { data, isLoading, isSuccess, error, isError }] = useRateCourseMutation();
	...
	const onSubmit = async () => {
		try {
			...
			notify({
				title: "Error",
				description: "An error occurred while rating this course. If you've rated this course before, you can only rate it once",
				type: "error",
			});
		}
	};
	...
	const getFillBlank = (body: FillInTheBlankBody) => {
		...
		if (body) {
			const [beforeDash, afterDash] = body?.question?.split(body?.answers[0]?.blankSection);
				...
				<div className=" block lg:grid lg:grid-cols-[25vw,_1fr] gap-2 lg:fullscreen:grid-cols-1 ">
                  ...
                  /> */}
											<VideoCoursePlayer
												...
												src={activeResource.videoUrl}
												onVideoEnded={moveToNextResource}
												startFrom={activeResource.startFrom ? "" + activeResource.startFrom : "0"}
												endAt={activeResource.endAt ? "" + activeResource.endAt : "0"}
											/>
										</div>
									</div>
								) : null}
