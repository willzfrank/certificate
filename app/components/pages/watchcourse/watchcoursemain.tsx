...
const WatchCourseMain = () => {
  ...
  const [pricingPlan, setPricingPlan] = React.useState<PricingPlan>(
    courseDetails?.pricings[0] ?? freePlan
  );
    ...
    if (discountDetails?.hasAppliedDiscount) {
      ...
      updatedPrice =
        discountDetails.type === 'DiscountByAbsoluteValue'
          ? discountDetails.value
          : originalPrice -
            Math.floor(originalPrice * (discountDetails.value / 100) ?? 0); // Use a default value if discountDetails or discountDetails.value is undefined
    }
  ...
  React.useEffect(() => {
    const paymentRequired =
      courseDetails?.modules[activeModuleIndex]?.paymentRequired;
  ...
  const moveToNextResource = React.useCallback(() => {
    ...
    if (activeResourceIndex === allResourses.length - 1) {
      let modulesLength = courseDetails?.modules?.length
        ? courseDetails?.modules?.length - 1
        : 0;
  ...
  const courseCompletionData = useCourseCompletion(
    courseId,
          ...
          <div className="hidden lg:block ">
            ...
            <div className=" h-screen overflow-x-hidden overflow-y-auto">
              <CourseContent
                ...
                courseType={
                  courseDetails?.pricings[0]?.price === 0 ? 'free' : 'paid'
                }
                ...
                {(!page || page === 'coursecontent') && (
                  <div className="lg:hidden">
                    <CourseContent
                      ...
                      ref={courseContentRef}
                      courseType={
                        courseDetails?.pricings[0]?.price === 0
                          ? 'free'
                          : 'paid'
                      }
                      subscribed={courseDetails?.isSubscribed as boolean}
