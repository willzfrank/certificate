...
const useCourseCompletion = (
  ...
  const [markCourseAsCompleted, { isSuccess }] =
  ...
  React.useEffect(() => {
    ...
    async function markCompleted() {
      const resources = await Promise.all(
        courseModulesIds.map((id) =>
          getModuleResources({ courseId, moduleId: id }).unwrap()
        )
      );
      const allVideos = resources
        .map((resource) => resource.data.videos)
        .flat();
      const allAssessments = resources
        .map((resource) => resource.data.assessments)
        .flat();
      const allDocuments = resources
        .map((resources) => resources.data.documents)
        .flat();
